      ...
      {   
          items.map(item=>(
            <div key={item.id} className="hover:scale-105 transition duration-200  ease-in-out w-72 md:mx-2 my-4 mx-auto  bg-opacity-40 bg-gradient-to-tr ring-2 ring-transparent hover:ring-gray-100  rounded-xl flex flex-col ">
                 <AnimateWhenInView duration={0.4} delay={1}  Iopacity={0} Fopacity={1} Is={0.7} Fs={1} Ix={50} Iy={10} Fy={0} Fx={0} >   
                  ...
                  <div className="w-72 rounded-b-xl  bg-black text-slate-50 shadow-lg ">
                        ...
                        <div className="mt-2 px-4 transition duration-150 ease-in-out justify-start space-x-3 hover:scale-105"><a className="flex flex-row" href={`https://www.youtube.com/channel/${item.snippet.videoOwnerChannelId}`}><p className="font-medium text-base truncate mb-2 my-auto">From: {item.snippet.videoOwnerChannelTitle}</p><img className=" w-5 h-5 ml-2" src="./svg/share_move_black.svg" /></a>  </div>   
                        <a className=" px-4" target="_blank" rel="noreferrer" href={"https://www.youtube.com/watch?v="+item.snippet.resourceId.videoId}>       
                        <p className="px-4 text-gray-300">Last Updated : {formatDate(new Date(item.snippet.publishedAt))}</p>
                        <div className="mt-20 flex flex-row justify-center rounded-b-xl items-center hover:bg-slate-200 text-slate-100 hover:text-black transition duration-200 ease-in bg-black">
